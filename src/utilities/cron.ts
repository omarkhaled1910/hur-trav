...
let liveStreamJob: ScheduledTask | null = null
...
export function startCronJobs(): void {
  ...
  // Start live stream fetching job (every 10 minutes)
  if (!liveStreamJob) {
    liveStreamJob = cron.schedule('*/10 * * * *', async () => {
      console.log('[Cron] Running live stream pipeline at', new Date().toISOString())
      try {
        await runLiveStreamPipeline()
      } catch (error) {
        console.error('[Cron] Live stream pipeline error:', error)
      }
    })
    console.log('[Cron] Live stream pipeline scheduled every 10 minutes')
  }
...
export function stopCronJobs(): void {
  ...
  if (liveStreamJob) {
    liveStreamJob.stop()
    liveStreamJob = null
    console.log('[Cron] Live stream job stopped')
  }
}
...
/**
 * Live stream pipeline: fetch live stream transcripts and create articles
 * Runs for authors with fetchLives enabled
 */
export async function runLiveStreamPipeline(): Promise<{
  processed: number
  articles: number
  errors: number
}> {
  ...
  try {
    ...
    if (authors.length === 0) {
      console.log('[LiveStream] No authors with fetchLives enabled')
      return { processed, articles, errors }
    }

    console.log(`[LiveStream] Processing ${authors.length} authors with fetchLives enabled`)
    ...
    for (const author of authors) {
      try {
        ...
        const knownLiveStreamIds = new Set(existingVideoDocs.map((v) => v.videoId as string))

        const videos = await fetchChannelVideos(author.channelId, 10, knownLiveStreamIds)
        ...
        for (const videoData of videos) {
          try {
            ...
            if (!transcript) {
              ...
              console.log(`[LiveStream] No transcript for: ${videoData.title}`)
              continue
            }
            ...
            if (!process.env.NEXT_PRIVATE_OPENAI_API_KEY) {
              console.warn('[LiveStream] OPENAI_API_KEY not set, skipping article generation')
              continue
            }
            ...
            articles++
            console.log(`[LiveStream] Article created from live: "${generatedArticle.title}"`)
          } catch (videoError) {
            errors++
            console.error(`[LiveStream] Error processing video ${videoData.videoId}:`, videoError)
          }
        }
      } catch (channelError) {
        errors++
        console.error(`[LiveStream] Error processing author ${author.name}:`, channelError)
      }
    }

    console.log(
      `[LiveStream] Complete: ${processed} videos processed, ${articles} articles created, ${errors} errors`,
    )
  } catch (error) {
    console.error('[LiveStream] Critical error:', error)
    errors++
  }
